...
fnc_attk_strigoi = {
	...
	[[_strigoi,_tgt_casp],"modules\AL_strigoi\atk_viz.sqf"] remoteExec ["execVM"];
	if (isPlayer _tgt_casp) then {
		[[_damage_strig],"modules\AL_strigoi\tgt_attk.sqf"] remoteExec ["execVM",_tgt_casp]
	} else {
...
fnc_show_strig = {
	...
	[[_strigoi],"modules\AL_strigoi\strigoi_sfx.sqf"] remoteExec ["execVM",0];
